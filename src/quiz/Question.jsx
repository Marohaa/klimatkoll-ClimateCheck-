...
const Question = (props) => {
 ...
 return (
      ...
      <div className="py-3 px-3 border rounded shadow" style={{
        backgroundColor: '#F6F6F2'
      }}>
        <div className="my-4 py-4 px-5 card-body border rounded " style={{ backgroundColor: 'white'}}>
        <h4>Fråga {props.index + 1}</h4>
        <h6>{currentQuestion.question}</h6>
        </div>
   
        {currentQuestion.all_answers.map((answer) => (
          <div className="form-check my-2 py-3 px-2 card-body border rounded" style={{ backgroundColor: 'white'}}>
          <input
             className="success"
             type="checkbox"
